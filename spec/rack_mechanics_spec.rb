...
describe "Shopping Cart Rack App" do
  ...
  describe "/cart" do
    xit "responds with empty cart message if the cart is empty" do
      Application.class_variable_set(:@@cart, [])
      get '/cart'
      expect(last_response.body).to include("Your cart is empty")
    end

    xit "responds with a cart list if there is something in there" do
  ...
  describe "/add" do
    ...
    xit 'Will add an item that is in the @@items list' do
    ...
    xit 'Will not add an item that is not in the @@items list' do
      Application.class_variable_set(:@@items, ["Figs","Oranges"])
      get '/add?item=Apples'
